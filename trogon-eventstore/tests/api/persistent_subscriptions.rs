...
async fn test_persistent_subscription(client: &Client) -> trogon_eventstore::Result<()> {
    ...
    let handle = tokio::spawn(async move {
        ...
        Ok::<usize, trogon_eventstore::Error>(count)
    });
...
async fn test_persistent_subscription_info_with_connection_details(
    ...
    let append_handle: tokio::task::JoinHandle<trogon_eventstore::Result<()>> =
    ...
    let ack_handle: tokio::task::JoinHandle<trogon_eventstore::Result<()>> =
        tokio::spawn(async move {
            loop {
                let event = sub.next().await?;
                sub.ack(&event).await?;
